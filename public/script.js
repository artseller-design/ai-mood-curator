document.addEventListener("DOMContentLoaded", () => {
  ...
  async function handleRecommend() {
    ...
    recommendBtn.textContent = "AI가 추천을 분석하는 중...";
  ...
  // Enter 키 단축키 지원 (감정 입력창에서 Enter, 상황 입력창에서 Ctrl+Enter)
  ...
  function showResult(data) {
    ...
    resultBox.innerHTML = `
      <h2>감정 요약</h2>
      <p>${escapeHtml(data.emotion_summary) || "감정 요약이 없습니다."}</p>

      <h2>추천 음악</h2>
      ${musicList.length > 0
        ? musicList.map(song => `
            <div class="item">
              <strong>${escapeHtml(song.title) || "제목 없음"}</strong>
              <span> - ${escapeHtml(song.artist) || "아티스트 없음"}</span>
              <p>${escapeHtml(song.reason) || "추천 이유가 없습니다."}</p>
            </div>
          `).join("")
        : "<p>추천 음악이 없습니다.</p>"
      }
      ...
      ${artList.length > 0
        ? artList.map(art => `
            <div class="item">
              <strong>${escapeHtml(art.title) || "작품명 없음"}</strong>
              <span> - ${escapeHtml(art.artist) || "작가 없음"}</span>
              <p>${escapeHtml(art.reason) || "추천 이유가 없습니다."}</p>
            </div>
          `).join("")
        : "<p>추천 미술 작품이 없습니다.</p>"
      }
